        ...
        filter_Posts_By_User_Id.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filter_Posts_By_User_Id.map((post) => (
              <article
                key={post.id}
                className="relative group flex flex-col h-full border-gray-800 bg-gray-900/95 p-5 shadow-lg shadow-black/30 hover:shadow-xl hover:-translate-y-1 transition duration-200 overflow-hidden rounded-lg"
              >
                {/* Title */}
                ...
                {/* Preview */}
                <p className="text-sm text-gray-300 flex-1 mb-3 line-clamp-3 group-hover:opacity-0 transition duration-200">
                  {post.content}
                </p>

                {/* Fade hint */}
                <div className="absolute bottom-0 left-0 right-0 h-10 bg-gradient-to-t from-gray-900 to-transparent group-hover:hidden" />
                ...
                <div className="absolute inset-0 bg-gray-900/95 backdrop-blur-sm p-5 opacity-0 group-hover:opacity-100 transition duration-300 overflow-y-auto">
